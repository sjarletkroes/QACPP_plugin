...
public class PRQAFrameworkInstallationConfiguration extends ToolInstallation implements PRQAToolSuite {
    ...
    public HashMap<String, String> createEnvironmentVariables(String workspaceRoot) {
        ...
        environment.put(PRQA_QACli.PRQAF_INSTALL_PATH, getHome());
        environment.put(PRQA_QACli.WORKSPACE_PATH, workspaceRoot);
        return environment;
    }
    ...
    public static final class DescriptorImpl extends ToolDescriptor<PRQAFrameworkInstallationConfiguration> {
        ...
        public boolean configure(StaplerRequest req, JSONObject json) throws FormException {
            ...
            if (!json.isEmpty()) {
                List<String> prqaInstallationNames = new ArrayList<String>();
                if (json.get(tool) instanceof JSONArray) {
                    JSONArray array = json.getJSONArray(tool);
                    Iterator it = array.iterator();
                    while (it.hasNext()) {
                        JSONObject jsonObject = (JSONObject) it.next();
                        String prqaName = jsonObject.getString(prqafName);
                        String prqaHome = jsonObject.getString(prqafHome);
                        if (Strings.isNullOrEmpty(prqaName) || Strings.isNullOrEmpty(prqaHome) || prqaInstallationNames.contains(prqaName) || !isValidString(prqaName)
                                || !isValidString(prqaHome)) {
                            it.remove();
                        } else {
                            prqaInstallationNames.add(prqaName);
                        }
                    }
                    if (array.isEmpty()) {
                        json = new JSONObject();
        ...
        public ListBoxModel doFillPRQafNameItems() {
            ...
            for (PRQAFrameworkInstallationConfiguration suite : getPRQaInstallations()) {
                model.add(suite.getName());
            }
            return model;
        }

        public PRQAFrameworkInstallationConfiguration[] getPRQaInstallations() {
            PRQAFrameworkInstallationConfiguration[] installations = Jenkins.getInstance()
                    .getDescriptorByType(PRQAFrameworkInstallationConfiguration.DescriptorImpl.class).getInstallations();
