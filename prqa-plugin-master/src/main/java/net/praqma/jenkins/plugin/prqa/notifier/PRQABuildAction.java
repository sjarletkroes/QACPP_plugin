...
public class PRQABuildAction implements Action {
     ...
     * Returns null in cases where it is not possible.
     *
     * This check is needed if you for some reason decide to switch report type
     * on the same job, since each report has it's own implementation of a
     * status. We need to do a check on all the collecte results to get those
     * that fits the current job profile.
     *
     * @param <T>
     * @param clazz
     * @return
    ...
    /**
     * Fetches the previous PRQA build. Skips builds that were not configured as
     * a PRQA Build.
     *
    ...
    /**
     * Determines weather to draw threhshold graphs. Uses the most recent build
     * as base.
     *
     * @param req
     * @param rsp
     ...
     *
     * After choosing your report type, as set of supported graphs are given,
     * which it is up to the user to add. Currently this is done
     * programatically, but given my design, it should be relatively simple to
     * make this possible to edit in the GUI.
     *
     * If a result is fetched and it does not contain the property to draw the
     * graphs the report demands we simply skip it. This means you can switch
     * report type in a job. You don't need to create a new job if you just want
     * to change reporting mode.
     *
     * This method catches the PrqaReadingException, when that exception is
     * thrown it means that the we skip the reading and continue.
     *
     * @param req
     * @param rsp
     * @throws IOException
