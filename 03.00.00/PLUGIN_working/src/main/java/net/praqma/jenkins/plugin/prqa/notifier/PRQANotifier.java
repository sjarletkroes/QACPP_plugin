...
public class PRQANotifier extends Publisher {
    ...
    //public final PostBuildActionSetup sourcePRQAFramework;
    ...
    private void copyReportsToArtifactsDir(ReportSettings settings, AbstractBuild<?, ?> build) throws IOException,
        ...
        if (settings instanceof PRQAReportSettings) {
            ...
            String pattern = "**/Warning Summary.txt";
            FilePath[] files = build.getWorkspace().list(pattern);
            if (files.length >= 1) {
                outStream.println(Messages.PRQANotifier_FoundReport("Warning Summary.txt"));
                String artifactDir = build.getArtifactsDir().getPath();
    ...
    public boolean perform(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener)
        ...
        if (sourceQAFramework != null && sourceQAFramework instanceof QAFrameworkPostBuildActionSetup) {
            ...
            // initialises  the report settings with project file
            ...
            // initialises  the report settings with filelist
                ...
                settings = new PRQAReportSettings(prqaReportPRQAToolSource.chosenServer, flSource.fileList,
                        ...
                        prqaReportPRQAToolSource.enableDependencyMode, prqaReportPRQAToolSource.enableDataFlowAnalysis,
                        prqaReportPRQAToolSource.generateCodeReviewReport, prqaReportPRQAToolSource.generateSuppressionReport,
                        prqaReportPRQAToolSource.generateWarningSummary, chosenReportTypes, productUsed, flSource.settingsFile);
            ...
            // QAR project file or file list:	.prj
            ...
            try {
                ...
                //QA·C(++) OK - QAC(++) Deep Flow Static Analyser -version- -date- for -system-
    ...
    public static final class DescriptorImpl extends BuildStepDescriptor<Publisher> {
        ...
        public Publisher newInstance(StaplerRequest req, JSONObject formData) throws Descriptor.FormException {
            ...
            final String WARNING_SUMMARY = "generateWarningSummary";
            ...
            if (sourceObject.has("fileProjectSource")) {
                JSONObject fileProject = sourceObject.getJSONObject("fileProjectSource");
                if (fileProject.containsKey(WARNING_SUMMARY)) {
                    if (fileProject.getBoolean(WARNING_SUMMARY)) {
                        instance.chosenReportTypes.add(QARReportType.WarningSummary);
                    }
                }
            }
            if (instance.getGraphTypes() == null || instance.getGraphTypes().isEmpty()) {
                ...
                if (instance.getChosenReportTypes().contains(QARReportType.Quality)) {
    ...
    private boolean performQaFrameworkBuild(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener) {
        ...
        outStream.println("_______________________ 1 _______________________");
        ...
        outStream.println("_______________________ 2 _______________________");

        outStream.println(Messages.PRQANotifier_ReportGenerateText());
        outStream.println("workspace location " + build.getWorkspace().getRemote());
        
        outStream.println("_______________________ 3 _______________________");
        ...
        outStream.println("_______________________ 4 _______________________");
        ...
        outStream.println("_________________________________________________");
        outStream.println("________" + qaVerifySettings.toString());
        outStream.println("________" + report.toString());
        outStream.println("________" + remoteToolCheck.toString());
        outStream.println("________" + remoteReport.toString());
        outStream.println("_________________________________________________");
        PRQAComplianceStatus currentBuild;
        
        outStream.println("______________________# 5 #______________________");
        
        try {
            currentBuild = performBuild(build, appSettings, remoteToolCheck, remoteReport, qaReportSettings);
        outStream.println("_______________________ 6 _______________________");
        } catch (PrqaException ex) {
        outStream.println("_______________________ 7 _______________________");
            log.log(Level.WARNING, "PrqaException", ex.getMessage());
            return false;
        }
        outStream.println(currentBuild.toString());
        
        outStream.println("_______________________ 8 _______________________");
    ...
    private boolean performPRQaFrameworkBuild(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener) {
    ...
    private PRQAComplianceStatus performBuild(AbstractBuild<?, ?> build, PRQAApplicationSettings appSettings,
        ...
        try {
            FilePath f = build.getWorkspace();
            QaFrameworkVersion qaFrameworkVersion = new QaFrameworkVersion(f.act(remoteToolCheck));
