...
public class PRQAReport implements Serializable {
    ...
    public CmdResult warningSummary(boolean isUnix) throws PrqaException {
        String finalCommand = createWarningSummaryCommand(isUnix);
        return executeCommand(finalCommand, "warningSummary");
    }

    public CmdResult executeCommand(String finalCommand, String type) throws PrqaException {
        Map<String, String> systemVars = new HashMap<String, String>();
        systemVars.putAll(System.getenv());
        CmdResult res = null;
        try {
            if (getEnvironment() == null) {
                PRQAReport._logEnv("Current " + type + " generation execution environment", systemVars);
                res = CommandLine.getInstance().run(finalCommand, workspace, true, false);
            } else {
                systemVars.putAll(getEnvironment());
                PRQAReport._logEnv("Current modified " + type + " generation execution environment", systemVars);
                res = CommandLine.getInstance().run(finalCommand, workspace, true, false, getEnvironment());
            }
        } catch (AbnormalProcessTerminationException abnex) {
            throw new PrqaException(String.format("Failed to " + type + ", message was:\n %s", abnex.getMessage()), abnex);
        }
        return res;
    }
    ...
    public String createReportCommand(boolean isUnix) throws PrqaException {
        ...
        for (PRQAContext.QARReportType type : settings.chosenReportTypes) {
            if (!type.equals(PRQAContext.QARReportType.WarningSummary)) {
    ...
    public String createWarningSummaryCommand(boolean isUnix) throws PrqaException {
        // Only works with filelist.lst & settings.via !!!
        if (!StringUtils.isBlank(settings.fileList)) {
            PRQACommandBuilder builder = new PRQACommandBuilder(appSettings != null ? PRQACommandBuilder.wrapInQuotationMarks(appSettings.productHome + "\\bin\\errsum.exe") : "ERRORERRORERROR");
            builder.prependArgument(settings.getProduct());
            builder.appendArgument(PRQACommandBuilder.getCmaf(workspace.getPath(), true));
            builder.appendArgument("-list " + PRQACommandBuilder.wrapInQuotationMarks(settings.fileList));
            builder.appendArgument("-via " + PRQACommandBuilder.getProjectFile(resolveAbsOrRelativePath(workspace, settings.settingsFile)));

            builder.appendArgument(PRQACommandBuilder.getDataFlowAnanlysisParameter(settings.enableDataFlowAnalysis));

            String output = String.format("%s\\Warning Summary.txt", workspace.getPath().replace(" ", "\\ "), this.workspace.getName());
            builder.appendArgument("> " + PRQACommandBuilder.wrapInQuotationMarks(output));
            return builder.getCommand();
        } else {
            throw new PrqaException("Report source not configured (File List)");
        }
    }
